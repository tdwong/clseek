...
#define _COPYRIGHT_	"(c) 2003-2012 Tzunghsing David <wong>"
#define _DESCRIPTION_	"Command-line seek utility"
#define _PROGRAMNAME_	"CLSeek"	/* program name */
#define _PROGRAMVERSION_	"1.2g"	/* program version */
		...
		switch (optcode) {
			...
			case 'a':
					if (optptr[0] == 'd') {
						...
						gEntityAttribute |= (ENTITY_FILE | ENTITY_DIRECTORY | ENTITY_DETAILS);
					}
					else {
...
// binary to string
	sprintf(sCompact, "%u", value);
	//-dbg- printf("value=%u, sCompact=%s\n", value, sCompact);
